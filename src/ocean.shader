...
uniform vec4 wave_4 = vec4(0.94, 0.20, 0.20, 30.4);
uniform vec4 wave_5 = vec4(0.30, 0.60, 0.19, 60.4);
uniform vec4 wave_6 = vec4(0.10, 0.95, 0.25, 45.4);
...
vec3 wave(vec2 pos, float time) {
	...
	to_return += gernster_wave(wave_5, pos, time);
	to_return += gernster_wave(wave_6, pos, time);
